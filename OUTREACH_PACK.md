# VoiceBrief — VA Outreach 私信话术包（手动执行）

> 目标：海外（只做国外）。找虚拟助理（VA，Virtual Assistant）一对一私信，确认"客户语音消息太多跟不住"这个痛点是不是真愿意付 $19/月。
> 为什么走这条路：Reddit 评论被 karma 墙挡了（comment_karma=0，AutoMod 自动删）。outreach 私信不撞这堵墙，是出第一笔钱最快的。
...
> 每天挑 **8–12 个**真人（看简介像真在接 VA 活的），不要群发同一条——每条都要像手打的。

---

## 二、3 套私信话术（英文，直接复制，按需改 1–2 个词更像你）
...
### 话术 B：痛点共鸣型（适合对方简介提到 "inbox zero / client comms"）
```
...
## 四、执行步骤（手动，因为自动化被 karma/指纹挡）

1. 打开上面任一平台，挑 8–12 个真 VA（简介有 "VA / virtual assistant / remote support"）。
2. 复制话术 A/B/C 之一，改 1–2 个词（比如把 "freelancer" 换成 "VA"）让像手打。
3. 发私信。**等回复**，对方显出痛点再轻提工具。
4. 填跟踪表。每天做一轮，连续 5–7 天看转化。
5. 出现要链接的 → 发 GitHub Pages 网址 → 他试 → 订阅 $19。
